...
public class NavGrid : IDisposable {
    ...
    public void AppendGeometry(Span<Int2> polygon, int delta) {
        ...
        for (int y = aabbMin.Y; y <= aabbMax.Y; y++) {
            ...
            gridIt.ComputeXRange(polygon, yPos, xCoords);
            for (int iX = 0; iX < xCoords.Count; iX += 2) {
                var xMin = xCoords[iX];
                var xMax = xCoords[iX + 1];
                for (int x = xMin; x < xMax; x++) {
                    map[x + y * Size.X] = (byte)(map[x + y * Size.X] + delta);
                }
            }
            xCoords.Clear();
        }
    ...
    public struct GridIteratorUtility {
        ...
        public void ComputeXRange(Span<Int2> polygon, int yPos, PooledList<int> xCoords) {
    ...
    public struct AdjacencyPushJob {
                    ...
                    for (int iy = 1; iy < Size.Y; iy++) {
                        for (int ix = 0; ix < Size.X; ix++) {
                            ...
                            var adjacency = new AdjacencyIds() {
                                Type1 = new TriangleType(ConvertTypeId(pntT0)),
                                Type2 = new TriangleType(ConvertTypeId(pntT1)),
                            };
        ...
        private byte ConvertTypeId(byte typeId) {
            if ((typeId & 0x0f) != 0) typeId = 0;
            else if ((typeId & 0xf0) != 0) typeId = (byte)(typeId & 0xf0);
            else typeId = 1;
            //if ((typeId & 0xff) != 0) typeId &= 0xf0; else typeId |= 0x01;
            return typeId;
        }
    }
