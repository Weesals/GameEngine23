...
inline XMVECTOR XM_CALLCONV _XMLoadFloat3(const Vector3* pSource) noexcept {
    return DirectX::XMLoadFloat3((const XMFLOAT3*)pSource);
}
_Use_decl_annotations_ inline XMVECTOR XM_CALLCONV _XMLoadFloat4(const Vector4* pSource) noexcept {
    return DirectX::XMLoadFloat4((const XMFLOAT4*)pSource);
}
_Use_decl_annotations_ inline XMMATRIX XM_CALLCONV _XMLoadFloat4x4(const Matrix* pSource) noexcept {
    return DirectX::XMLoadFloat4x4((const XMFLOAT4X4*)pSource);
}


_Use_decl_annotations_ inline void XM_CALLCONV XMStoreFloat2(Vector2* pDestination, FXMVECTOR V) noexcept {
    DirectX::XMStoreFloat2((XMFLOAT2*)pDestination, V);
}
_Use_decl_annotations_ inline void XM_CALLCONV XMStoreFloat3(Vector3* pDestination, FXMVECTOR V) noexcept {
    DirectX::XMStoreFloat3((XMFLOAT3*)pDestination, V);
}
_Use_decl_annotations_ inline void XM_CALLCONV XMStoreFloat4(Vector4* pDestination, FXMVECTOR V) noexcept {
    DirectX::XMStoreFloat4((XMFLOAT4*)pDestination, V);
}
_Use_decl_annotations_ inline Vector2 XM_CALLCONV XMStoreFloat2(FXMVECTOR V) noexcept {
    Vector2 pDestination;
    DirectX::XMStoreFloat2((XMFLOAT2*)&pDestination, V);
    return pDestination;
}
_Use_decl_annotations_ inline Vector3 XM_CALLCONV XMStoreFloat3(FXMVECTOR V) noexcept {
    Vector3 pDestination;
    DirectX::XMStoreFloat3((XMFLOAT3*)&pDestination, V);
    return pDestination;
}
_Use_decl_annotations_ inline Vector4 XM_CALLCONV XMStoreFloat4(FXMVECTOR V) noexcept {
    Vector4 pDestination;
    DirectX::XMStoreFloat4((XMFLOAT4*)&pDestination, V);
    return pDestination;
}

_Use_decl_annotations_ inline void XM_CALLCONV _XMStoreFloat4x4(Matrix* pDestination, FXMMATRIX M) noexcept {
    XMStoreFloat4x4((XMFLOAT4X4*)pDestination, M);
}
...
{
    using namespace DirectX;
    const XMVECTOR v1 = _XMLoadFloat2(this);
    const XMVECTOR v2 = _XMLoadFloat2(&V);
...
namespace DirectX
{
    namespace SimpleMath
    {
        ...
        {
            using namespace DirectX;
            const XMVECTOR v1 = _XMLoadFloat2(&V1);
            const XMVECTOR v2 = _XMLoadFloat2(&V2);
            const XMVECTOR X = XMVectorAdd(v1, v2);
            Vector2 R;
            XMStoreFloat2(&R, X);
            return R;
        ...
        {
            using namespace DirectX;
            const XMVECTOR v1 = _XMLoadFloat2(&V);
            const XMVECTOR X = XMVectorScale(v1, S);
            Vector2 R;
            XMStoreFloat2(&R, X);
            return R;
...
bool Vector2::InBounds(const Vector2& Bounds) const noexcept
{
    using namespace DirectX;
    const XMVECTOR v1 = _XMLoadFloat2(this);
    const XMVECTOR v2 = _XMLoadFloat2(&Bounds);
...
Vector2 Vector2::Clamp(const Vector2& V, const Vector2& vmin, const Vector2& vmax) noexcept
{
    using namespace DirectX;
    const XMVECTOR v1 = _XMLoadFloat2(&V);
    const XMVECTOR v2 = _XMLoadFloat2(&vmin);
    const XMVECTOR v3 = _XMLoadFloat2(&vmax);
...
{
    using namespace DirectX;
    const XMVECTOR x1 = _XMLoadFloat2(&v1);
    const XMVECTOR x2 = _XMLoadFloat2(&v2);
...
void Vector2::Barycentric(const Vector2& v1, const Vector2& v2, const Vector2& v3, float f, float g, Vector2& result) noexcept
{
    ...
    const XMVECTOR x1 = _XMLoadFloat2(&v1);
    const XMVECTOR x2 = _XMLoadFloat2(&v2);
    const XMVECTOR x3 = _XMLoadFloat2(&v3);
...
void Vector2::CatmullRom(const Vector2& v1, const Vector2& v2, const Vector2& v3, const Vector2& v4, float t, Vector2& result) noexcept
{
    ...
    const XMVECTOR x2 = _XMLoadFloat2(&v2);
    const XMVECTOR x3 = _XMLoadFloat2(&v3);
    const XMVECTOR x4 = _XMLoadFloat2(&v4);
...
void Vector2::Hermite(const Vector2& v1, const Vector2& t1, const Vector2& v2, const Vector2& t2, float t, Vector2& result) noexcept
{
    using namespace DirectX;
    const XMVECTOR x1 = _XMLoadFloat2(&v1);
    const XMVECTOR x2 = _XMLoadFloat2(&t1);
    const XMVECTOR x3 = _XMLoadFloat2(&v2);
    const XMVECTOR x4 = _XMLoadFloat2(&t2);
...
{
    using namespace DirectX;
    const XMVECTOR i = _XMLoadFloat2(&ivec);
    const XMVECTOR n = _XMLoadFloat2(&nvec);
...
void Vector2::Transform(const Vector2& v, const Quaternion& quat, Vector2& result) noexcept
{
    using namespace DirectX;
    const XMVECTOR v1 = _XMLoadFloat2(&v);
    const XMVECTOR q = _XMLoadFloat4(&quat);
...
void Vector2::Transform(const Vector2& v, const Matrix& m, Vector2& result) noexcept
{
    using namespace DirectX;
    const XMVECTOR v1 = _XMLoadFloat2(&v);
    const XMMATRIX M = _XMLoadFloat4x4(&m);
...
void Vector2::Transform(const Vector2* varray, size_t count, const Matrix& m, Vector2* resultArray) noexcept
{
    using namespace DirectX;
    const XMMATRIX M = _XMLoadFloat4x4(&m);
    XMVector2TransformCoordStream((XMFLOAT2*)resultArray, sizeof(XMFLOAT2), (const XMFLOAT2*)varray, sizeof(XMFLOAT2), count, M);
}
...
void Vector2::Transform(const Vector2* varray, size_t count, const Matrix& m, Vector4* resultArray) noexcept
{
    using namespace DirectX;
    const XMMATRIX M = _XMLoadFloat4x4(&m);
    XMVector2TransformStream((XMFLOAT4*)resultArray, sizeof(XMFLOAT4), (const XMFLOAT2*)varray, sizeof(XMFLOAT2), count, M);
}
...
void Vector2::TransformNormal(const Vector2* varray, size_t count, const Matrix& m, Vector2* resultArray) noexcept
{
    using namespace DirectX;
    const XMMATRIX M = _XMLoadFloat4x4(&m);
    XMVector2TransformNormalStream((XMFLOAT2*)resultArray, sizeof(XMFLOAT2), (const XMFLOAT2*)varray, sizeof(XMFLOAT2), count, M);
}
...
{
    using namespace DirectX;
    const XMVECTOR v1 = _XMLoadFloat3(this);
    const XMVECTOR v2 = _XMLoadFloat3(&V);
...
namespace DirectX
{
    namespace SimpleMath
    {
        ...
        {
            using namespace DirectX;
            const XMVECTOR v1 = _XMLoadFloat3(&V1);
            const XMVECTOR v2 = _XMLoadFloat3(&V2);
            const XMVECTOR X = XMVectorAdd(v1, v2);
            Vector3 R;
            XMStoreFloat3(&R, X);
            return R;
        ...
        {
            using namespace DirectX;
            const XMVECTOR v1 = _XMLoadFloat3(&V);
            const XMVECTOR X = XMVectorScale(v1, S);
            Vector3 R;
            XMStoreFloat3(&R, X);
            return R;
...
bool Vector3::InBounds(const Vector3& Bounds) const noexcept
{
    using namespace DirectX;
    const XMVECTOR v1 = _XMLoadFloat3(this);
    const XMVECTOR v2 = _XMLoadFloat3(&Bounds);
...
Vector3 Vector3::Cross(const Vector3& V1, Vector3& V2) noexcept
{
    ...
    return XMStoreFloat3(R);
}

Vector3 Vector3::Clamp(const Vector3& value, const Vector3& vmin, const Vector3& vmax) noexcept
{
    using namespace DirectX;
    const XMVECTOR v1 = _XMLoadFloat3(&value);
    const XMVECTOR v2 = _XMLoadFloat3(&vmin);
    const XMVECTOR v3 = _XMLoadFloat3(&vmax);
    const XMVECTOR X = XMVectorClamp(v1, v2, v3);
    return XMStoreFloat3(X);
}
...
{
    using namespace DirectX;
    const XMVECTOR x1 = _XMLoadFloat3(&v1);
    const XMVECTOR x2 = _XMLoadFloat3(&v2);
    const XMVECTOR X = XMVectorMin(x1, x2);
...
void Vector3::Barycentric(const Vector3& v1, const Vector3& v2, const Vector3& v3, float f, float g, Vector3& result) noexcept
{
    ...
    const XMVECTOR x1 = _XMLoadFloat3(&v1);
    const XMVECTOR x2 = _XMLoadFloat3(&v2);
    const XMVECTOR x3 = _XMLoadFloat3(&v3);
    const XMVECTOR X = XMVectorBaryCentric(x1, x2, x3, f, g);
    XMStoreFloat3(&result, X);
}
...
void Vector3::CatmullRom(const Vector3& v1, const Vector3& v2, const Vector3& v3, const Vector3& v4, float t, Vector3& result) noexcept
{
    ...
    const XMVECTOR x2 = _XMLoadFloat3(&v2);
    const XMVECTOR x3 = _XMLoadFloat3(&v3);
    const XMVECTOR x4 = _XMLoadFloat3(&v4);
    const XMVECTOR X = XMVectorCatmullRom(x1, x2, x3, x4, t);
    XMStoreFloat3(&result, X);
}
...
void Vector3::Hermite(const Vector3& v1, const Vector3& t1, const Vector3& v2, const Vector3& t2, float t, Vector3& result) noexcept
{
    using namespace DirectX;
    const XMVECTOR x1 = _XMLoadFloat3(&v1);
    const XMVECTOR x2 = _XMLoadFloat3(&t1);
    const XMVECTOR x3 = _XMLoadFloat3(&v2);
    const XMVECTOR x4 = _XMLoadFloat3(&t2);
    const XMVECTOR X = XMVectorHermite(x1, x2, x3, x4, t);
    XMStoreFloat3(&result, X);
}
...
{
    using namespace DirectX;
    const XMVECTOR i = _XMLoadFloat3(&ivec);
    const XMVECTOR n = _XMLoadFloat3(&nvec);
...
void Vector3::Transform(const Vector3* varray, size_t count, const Matrix& m, Vector3* resultArray) noexcept
{
    using namespace DirectX;
    const XMMATRIX M = _XMLoadFloat4x4(&m);
    XMVector3TransformCoordStream((XMFLOAT3*)resultArray, sizeof(XMFLOAT3), (const XMFLOAT3*)varray, sizeof(XMFLOAT3), count, M);
}
...
void Vector3::Transform(const Vector3* varray, size_t count, const Matrix& m, Vector4* resultArray) noexcept
{
    using namespace DirectX;
    const XMMATRIX M = _XMLoadFloat4x4(&m);
    XMVector3TransformStream((XMFLOAT4*)resultArray, sizeof(XMFLOAT4), (const XMFLOAT3*)varray, sizeof(XMFLOAT3), count, M);
}
...
void Vector3::TransformNormal(const Vector3* varray, size_t count, const Matrix& m, Vector3* resultArray) noexcept
{
    using namespace DirectX;
    const XMMATRIX M = _XMLoadFloat4x4(&m);
    XMVector3TransformNormalStream((XMFLOAT3*)resultArray, sizeof(XMFLOAT3), (const XMFLOAT3*)varray, sizeof(XMFLOAT3), count, M);
}
...
Vector4 Vector4::Abs(const Vector4& v) noexcept
{
    return Vector4(v.x < 0 ? -v.x : v.x, v.y < 0 ? -v.y : v.y, v.z < 0 ? -v.z : v.z, v.w < 0 ? -v.w : v.w);
}
...
{
    using namespace DirectX;
    const XMVECTOR v1 = _XMLoadFloat4(this);
    const XMVECTOR v2 = _XMLoadFloat4(&V);
...
namespace DirectX
{
    namespace SimpleMath
    {
        ...
        {
            using namespace DirectX;
            const XMVECTOR v1 = _XMLoadFloat4(&V1);
            const XMVECTOR v2 = _XMLoadFloat4(&V2);
            const XMVECTOR X = XMVectorAdd(v1, v2);
            Vector4 R;
            XMStoreFloat4(&R, X);
            return R;
        ...
        {
            using namespace DirectX;
            const XMVECTOR v1 = _XMLoadFloat4(&V);
            const XMVECTOR X = XMVectorScale(v1, S);
            Vector4 R;
            XMStoreFloat4(&R, X);
            return R;
...
bool Vector4::InBounds(const Vector4& Bounds) const noexcept
{
    using namespace DirectX;
    const XMVECTOR v1 = _XMLoadFloat4(this);
    const XMVECTOR v2 = _XMLoadFloat4(&Bounds);
...
{
    using namespace DirectX;
    const XMVECTOR x1 = _XMLoadFloat4(this);
    const XMVECTOR x2 = _XMLoadFloat4(&v1);
    const XMVECTOR x3 = _XMLoadFloat4(&v2);
...
void Vector4::Clamp(const Vector4& vmin, const Vector4& vmax) noexcept
{
    using namespace DirectX;
    const XMVECTOR v1 = _XMLoadFloat4(this);
    const XMVECTOR v2 = _XMLoadFloat4(&vmin);
    const XMVECTOR v3 = _XMLoadFloat4(&vmax);
    const XMVECTOR X = XMVectorClamp(v1, v2, v3);
    XMStoreFloat4(this, X);
}
...
{
    using namespace DirectX;
    const XMVECTOR x1 = _XMLoadFloat4(&v1);
    const XMVECTOR x2 = _XMLoadFloat4(&v2);
    const XMVECTOR V = XMVectorSubtract(x2, x1);
...
void Vector4::Barycentric(const Vector4& v1, const Vector4& v2, const Vector4& v3, float f, float g, Vector4& result) noexcept
{
    ...
    const XMVECTOR x1 = _XMLoadFloat4(&v1);
    const XMVECTOR x2 = _XMLoadFloat4(&v2);
    const XMVECTOR x3 = _XMLoadFloat4(&v3);
    const XMVECTOR X = XMVectorBaryCentric(x1, x2, x3, f, g);
    XMStoreFloat4(&result, X);
}
...
void Vector4::CatmullRom(const Vector4& v1, const Vector4& v2, const Vector4& v3, const Vector4& v4, float t, Vector4& result) noexcept
{
    ...
    const XMVECTOR x2 = _XMLoadFloat4(&v2);
    const XMVECTOR x3 = _XMLoadFloat4(&v3);
    const XMVECTOR x4 = _XMLoadFloat4(&v4);
    const XMVECTOR X = XMVectorCatmullRom(x1, x2, x3, x4, t);
    XMStoreFloat4(&result, X);
}
...
void Vector4::Hermite(const Vector4& v1, const Vector4& t1, const Vector4& v2, const Vector4& t2, float t, Vector4& result) noexcept
{
    using namespace DirectX;
    const XMVECTOR x1 = _XMLoadFloat4(&v1);
    const XMVECTOR x2 = _XMLoadFloat4(&t1);
    const XMVECTOR x3 = _XMLoadFloat4(&v2);
    const XMVECTOR x4 = _XMLoadFloat4(&t2);
    const XMVECTOR X = XMVectorHermite(x1, x2, x3, x4, t);
    XMStoreFloat4(&result, X);
}
...
{
    using namespace DirectX;
    const XMVECTOR i = _XMLoadFloat4(&ivec);
    const XMVECTOR n = _XMLoadFloat4(&nvec);
...
void Vector4::Transform(const Vector4* varray, size_t count, const Matrix& m, Vector4* resultArray) noexcept
{
    using namespace DirectX;
    const XMMATRIX M = _XMLoadFloat4x4(&m);
    XMVector4TransformStream((XMFLOAT4*)resultArray, sizeof(XMFLOAT4), (const XMFLOAT4*)varray, sizeof(XMFLOAT4), count, M);
}
...
DirectX::SimpleMath::Rectangle Rectangle::Intersect(const Rectangle& ra, const Rectangle& rb) noexcept
{
...
DirectX::SimpleMath::Rectangle Rectangle::Union(const Rectangle& ra, const Rectangle& rb) noexcept
{
    const long righta = ra.x + ra.width;
    const long rightb = rb.x + rb.width;
...
{
    using namespace DirectX;
    const XMVECTOR q1 = _XMLoadFloat4(this);
    const XMVECTOR q2 = _XMLoadFloat4(&q);
...
{
    using namespace DirectX;
    const XMVECTOR q = _XMLoadFloat4(this);
...
//------------------------------------------------------------------------------
        ...
        {
            using namespace DirectX;
            const XMVECTOR q1 = _XMLoadFloat4(&Q1);
            const XMVECTOR q2 = _XMLoadFloat4(&Q2);
        ...
        {
            using namespace DirectX;
            const XMVECTOR q = _XMLoadFloat4(&Q);
        ...
        {
            using namespace DirectX;
            const XMVECTOR q1 = _XMLoadFloat4(&Q);
...
{
    using namespace DirectX;
    const XMVECTOR a = _XMLoadFloat3(&axis);
...
Quaternion Quaternion::CreateFromYawPitchRoll(const Vector3& angles) noexcept
{
    using namespace DirectX;
    Quaternion R;
    XMStoreFloat4(&R, XMQuaternionRotationRollPitchYawFromVector(_XMLoadFloat3(&angles)));
    return R;
}
...
{
    using namespace DirectX;
    const XMMATRIX M0 = _XMLoadFloat4x4(&M);
...
{
    using namespace DirectX;
    const XMVECTOR Q0 = _XMLoadFloat4(&q1);
    const XMVECTOR Q1 = _XMLoadFloat4(&q2);
...
{
    const XMVECTOR T = _XMLoadFloat4(this);

    // We can use the conjugate here instead of inverse assuming q1 & q2 are normalized.
    const XMVECTOR R = XMQuaternionMultiply(XMQuaternionConjugate(T), _XMLoadFloat4(&target));
...
void Quaternion::FromToRotation(const Vector3& fromDir, const Vector3& toDir, Quaternion& result) noexcept
{
    ...
    const XMVECTOR F = XMVector3Normalize(_XMLoadFloat3(&fromDir));
    const XMVECTOR T = XMVector3Normalize(_XMLoadFloat3(&toDir));
    ...
    {
        XMVECTOR axis = XMVector3Cross(F, _XMLoadFloat3(&Vector3::Right));
        if (XMVector3NearEqual(XMVector3LengthSq(axis), g_XMZero, g_XMEpsilon))
        {
            axis = XMVector3Cross(F, _XMLoadFloat3(&Vector3::Up));
        }
...
void Quaternion::LookRotation(const Vector3& forward, const Vector3& up, Quaternion& result) noexcept
{
    ...
    const XMVECTOR C = XMVector3Cross(_XMLoadFloat3(&forward), _XMLoadFloat3(&up));
    ...
    const XMVECTOR U = XMQuaternionMultiply(_XMLoadFloat4(&q1), _XMLoadFloat3(&Vector3::Up));

    Quaternion q2;
    FromToRotation(XMStoreFloat3(U), up, q2);

    XMStoreFloat4(&result, XMQuaternionMultiply(_XMLoadFloat4(&q2), _XMLoadFloat4(&q1)));
}
...
{
    using namespace DirectX;
    const XMMATRIX M1 = _XMLoadFloat4x4(this);
    const XMMATRIX M2 = _XMLoadFloat4x4(&M);
    const XMMATRIX X = XMMatrixMultiply(M1, M2);
    _XMStoreFloat4x4(this, X);
    return *this;
}
...
namespace DirectX
{
    namespace SimpleMath
    {
        ...
        {
            using namespace DirectX;
            const XMMATRIX m1 = _XMLoadFloat4x4(&M1);
            const XMMATRIX m2 = _XMLoadFloat4x4(&M2);
            const XMMATRIX X = XMMatrixMultiply(m1, m2);

            Matrix R;
            _XMStoreFloat4x4(&R, X);
            return R;
        }
...
bool Matrix::Decompose(Vector3& scale, Quaternion& rotation, Vector3& translation) noexcept
{
    ...
    if (!XMMatrixDecompose(&s, &r, &t, *(FXMMATRIX*)this))
        return false;
...
{
    using namespace DirectX;
    const XMMATRIX M = _XMLoadFloat4x4(this);
    Matrix R;
    _XMStoreFloat4x4(&R, XMMatrixTranspose(M));
    return R;
}
...
void Matrix::Transpose(Matrix& result) const noexcept
{
    using namespace DirectX;
    const XMMATRIX M = _XMLoadFloat4x4(this);
    _XMStoreFloat4x4(&result, XMMatrixTranspose(M));
}
...
Matrix Matrix::Invert() const noexcept
{
    ...
    Matrix R;
    XMVECTOR det;
    _XMStoreFloat4x4(&R, XMMatrixInverse(&det, M));
    return R;
}
...
void Matrix::Invert(Matrix& result) const noexcept
{
    ...
    const XMMATRIX M = _XMLoadFloat4x4(this);
    XMVECTOR det;
    _XMStoreFloat4x4(&result, XMMatrixInverse(&det, M));
}
...
{
    using namespace DirectX;
    const XMVECTOR O = _XMLoadFloat3(&object);
    const XMVECTOR C = _XMLoadFloat3(&cameraPosition);
    ...
    if (XMVector3Less(N, g_XMEpsilon))
    {
        if (cameraForward)
        {
            const XMVECTOR F = _XMLoadFloat3(cameraForward);
            Z = XMVectorNegate(F);
        }
        else
            Z = g_XMNegIdentityR2;
    ...
    const XMVECTOR up = _XMLoadFloat3(&cameraUp);
    ...
    const Vector3* objectForward) noexcept
{
    ...
    const XMVECTOR Y = _XMLoadFloat3(&rotateAxis);
    XMVECTOR X, Z;
...
Matrix Matrix::CreateTranslation(const Vector3& position) noexcept
{
    using namespace DirectX;
    Matrix R;
    _XMStoreFloat4x4(&R, XMMatrixTranslation(position.x, position.y, position.z));
    return R;
}
...
Matrix Matrix::CreateTranslation(float x, float y, float z) noexcept
{
    using namespace DirectX;
    Matrix R;
    _XMStoreFloat4x4(&R, XMMatrixTranslation(x, y, z));
    return R;
}
...
Matrix Matrix::CreateScale(const Vector3& scales) noexcept
{
    using namespace DirectX;
    Matrix R;
    _XMStoreFloat4x4(&R, XMMatrixScaling(scales.x, scales.y, scales.z));
    return R;
}
...
Matrix Matrix::CreateScale(float xs, float ys, float zs) noexcept
{
    using namespace DirectX;
    Matrix R;
    _XMStoreFloat4x4(&R, XMMatrixScaling(xs, ys, zs));
    return R;
}
...
Matrix Matrix::CreateScale(float scale) noexcept
{
    using namespace DirectX;
    Matrix R;
    _XMStoreFloat4x4(&R, XMMatrixScaling(scale, scale, scale));
    return R;
}
...
Matrix Matrix::CreateRotationX(float radians) noexcept
{
    using namespace DirectX;
    Matrix R;
    _XMStoreFloat4x4(&R, XMMatrixRotationX(radians));
    return R;
}
...
Matrix Matrix::CreateRotationY(float radians) noexcept
{
    using namespace DirectX;
    Matrix R;
    _XMStoreFloat4x4(&R, XMMatrixRotationY(radians));
    return R;
}
...
Matrix Matrix::CreateRotationZ(float radians) noexcept
{
    using namespace DirectX;
    Matrix R;
    _XMStoreFloat4x4(&R, XMMatrixRotationZ(radians));
    return R;
}
...
Matrix Matrix::CreateFromAxisAngle(const Vector3& axis, float angle) noexcept
{
    ...
    Matrix R;
    const XMVECTOR a = _XMLoadFloat3(&axis);
    _XMStoreFloat4x4(&R, XMMatrixRotationAxis(a, angle));
    return R;
}
...
Matrix Matrix::CreatePerspectiveFieldOfView(float fov, float aspectRatio, float nearPlane, float farPlane) noexcept
{
    using namespace DirectX;
    Matrix R;
    _XMStoreFloat4x4(&R, XMMatrixPerspectiveFovLH(fov, aspectRatio, nearPlane, farPlane));
    return R;
}
...
Matrix Matrix::CreatePerspective(float width, float height, float nearPlane, float farPlane) noexcept
{
    using namespace DirectX;
    Matrix R;
    _XMStoreFloat4x4(&R, XMMatrixPerspectiveLH(width, height, nearPlane, farPlane));
    return R;
}
...
Matrix Matrix::CreatePerspectiveOffCenter(float left, float right, float bottom, float top, float nearPlane, float farPlane) noexcept
{
    using namespace DirectX;
    Matrix R;
    _XMStoreFloat4x4(&R, XMMatrixPerspectiveOffCenterLH(left, right, bottom, top, nearPlane, farPlane));
    return R;
}
...
Matrix Matrix::CreateOrthographic(float width, float height, float zNearPlane, float zFarPlane) noexcept
{
    using namespace DirectX;
    Matrix R;
    _XMStoreFloat4x4(&R, XMMatrixOrthographicLH(width, height, zNearPlane, zFarPlane));
    return R;
}
...
Matrix Matrix::CreateOrthographicOffCenter(float left, float right, float bottom, float top, float zNearPlane, float zFarPlane) noexcept
{
    using namespace DirectX;
    Matrix R;
    _XMStoreFloat4x4(&R, XMMatrixOrthographicOffCenterLH(left, right, bottom, top, zNearPlane, zFarPlane));
    return R;
}
...
Matrix Matrix::CreateLookAt(const Vector3& eye, const Vector3& target, const Vector3& up) noexcept
{
    using namespace DirectX;
    Matrix R;
    const XMVECTOR eyev = _XMLoadFloat3(&eye);
    const XMVECTOR targetv = _XMLoadFloat3(&target);
    const XMVECTOR upv = _XMLoadFloat3(&up);
    _XMStoreFloat4x4(&R, XMMatrixLookAtLH(eyev, targetv, upv));
    return R;
}
...
{
    using namespace DirectX;
    const XMVECTOR zaxis = XMVector3Normalize(XMVectorNegate(_XMLoadFloat3(&forward)));
    XMVECTOR yaxis = _XMLoadFloat3(&up);
...
{
    using namespace DirectX;
    const XMVECTOR quatv = _XMLoadFloat4(&rotation);
    Matrix R;
    _XMStoreFloat4x4(&R, XMMatrixRotationQuaternion(quatv));
    return R;
}
...
Matrix Matrix::CreateFromYawPitchRoll(float yaw, float pitch, float roll) noexcept
{
    using namespace DirectX;
    Matrix R;
    _XMStoreFloat4x4(&R, XMMatrixRotationRollPitchYaw(pitch, yaw, roll));
    return R;
}
...
Matrix Matrix::CreateFromYawPitchRoll(const Vector3& angles) noexcept
{
    using namespace DirectX;
    Matrix R;
    _XMStoreFloat4x4(&R, XMMatrixRotationRollPitchYawFromVector(_XMLoadFloat3(&angles)));
    return R;
}
...
{
    using namespace DirectX;
    const XMVECTOR light = _XMLoadFloat3(&lightDir);
    const XMVECTOR planev = _XMLoadFloat4(&plane);
    Matrix R;
    _XMStoreFloat4x4(&R, XMMatrixShadow(planev, light));
    return R;
}
...
Matrix Matrix::CreateReflection(const Plane& plane) noexcept
{
    ...
    const XMVECTOR planev = _XMLoadFloat4(&plane);
    Matrix R;
    _XMStoreFloat4x4(&R, XMMatrixReflect(planev));
    return R;
}
...
void Matrix::Transform(const Matrix& M, const Quaternion& rotation, Matrix& result) noexcept
{
    ...
    _XMStoreFloat4x4(&result, XMMatrixMultiply(M0, M1));
}
...
{
    using namespace DirectX;
    const XMVECTOR P0 = _XMLoadFloat3(&point1);
    const XMVECTOR P1 = _XMLoadFloat3(&point2);
    const XMVECTOR P2 = _XMLoadFloat3(&point3);
...
{
    using namespace DirectX;
    const XMVECTOR P = _XMLoadFloat3(&point);
    const XMVECTOR N = _XMLoadFloat3(&normal);
...
{
    using namespace DirectX;
    const XMVECTOR p1 = _XMLoadFloat4(this);
    const XMVECTOR p2 = _XMLoadFloat4(&p);
...
{
    using namespace DirectX;
    const XMVECTOR p = _XMLoadFloat4(this);
...
float Plane::Dot(const Vector4& v) const noexcept
{
    using namespace DirectX;
    const XMVECTOR p = _XMLoadFloat4(this);
    const XMVECTOR v0 = _XMLoadFloat4(&v);
...
float Plane::DotCoordinate(const Vector3& position) const noexcept
{
    using namespace DirectX;
    const XMVECTOR p = _XMLoadFloat4(this);
    const XMVECTOR v0 = _XMLoadFloat3(&position);
...
float Plane::DotNormal(const Vector3& normal) const noexcept
{
    using namespace DirectX;
    const XMVECTOR p = _XMLoadFloat4(this);
    const XMVECTOR n0 = _XMLoadFloat3(&normal);
...
{
    using namespace DirectX;
    const XMVECTOR p = _XMLoadFloat4(&plane);
    const XMMATRIX m0 = _XMLoadFloat4x4(&M);
...
void Plane::Transform(const Plane& plane, const Quaternion& rotation, Plane& result) noexcept
{
    using namespace DirectX;
    const XMVECTOR p = _XMLoadFloat4(&plane);
    const XMVECTOR q = _XMLoadFloat4(&rotation);
...
{
    using namespace DirectX;
    const XMVECTOR c1 = _XMLoadFloat4(this);
    const XMVECTOR c2 = _XMLoadFloat4(&c);
...
{
    using namespace DirectX;
    const XMVECTOR c = _XMLoadFloat4(this);
...
//------------------------------------------------------------------------------
        ...
        {
            using namespace DirectX;
            const XMVECTOR c1 = _XMLoadFloat4(&C1);
            const XMVECTOR c2 = _XMLoadFloat4(&C2);
            Color R;
        ...
        {
            using namespace DirectX;
            const XMVECTOR c = _XMLoadFloat4(&C);
            Color R;
        ...
        {
            using namespace DirectX;
            const XMVECTOR c1 = _XMLoadFloat4(&C);
            Color R;
...
{
    using namespace DirectX;
    const XMVECTOR C0 = _XMLoadFloat4(&c1);
    const XMVECTOR C1 = _XMLoadFloat4(&c2);
...
//------------------------------------------------------------------------------
_Use_decl_annotations_
ColorB4::ColorB4(const float* pArray) noexcept
    : ColorB4(pArray[0], pArray[1], pArray[2], pArray[3])
{
}
...
{
    using namespace DirectX;
    XMVECTOR v = _XMLoadFloat3(&p);
    const XMMATRIX projection = _XMLoadFloat4x4(&proj);
    v = XMVector3Project(v, x, y, width, height, minDepth, maxDepth, projection, *(XMMATRIX*)&view, *(XMMATRIX*)&world);
    Vector3 result;
