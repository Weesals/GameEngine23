...
PSInput VSMain(VSInput input)
{
    ...
    result.position = mul(ModelViewProjection, float4(input.position.xyz, 1.0));
    result.viewPos = mul(ModelView, float4(input.position.xyz, 1.0));
    result.normal = mul(ModelView, float4(input.normal.xyz, 0.0));
    result.uv = input.uv;
